...
const Advantage = () => {
  ...
  return (
    <div className="min-h-full">
      <h1
        className={cn(
          "text-[2.5rem] leading-[3.5rem] mt-[6.5625rem] mb-[3.3125rem] text-center",
          IS_FIRST ? "text-[#9164FF]" : "text-[#E64921]"
        )}>
        我们的优势
      </h1>
      <ul className="w-full flex flex-col md:flex-row justify-center items-center space-y-[3.75rem] md:space-y-0 md:space-x-[3.75rem] pb-10">
        {advantageLists.map((_, idx) => (
          <li key={idx}>
            <AdvantageCard {..._} />
          </li>
        ))}
