...
import type { TQueryFormMap, TTableOptionsItem, TDeliveryPlansTableItem, TGetDeliveryPlansListProps, TDrawerType, TEmptyPromiseFn } from '@/type'
...
const getTableData = async () => {
  ...
  try {
    ...
    const { data: { content, totalElements } } = await getDeliveryPlansList(params)
    if (content) {
      tableData.length = 0
...
const openDrawer = ({
  ...
  switch (type) {
    case 'edit':
      params.id = id
      drawerTitle.value = '编辑投放计划'
      break
    case 'copy':
      params.id = id
      drawerTitle.value = '复制投放计划'
      break
    case 'new':
      drawerTitle.value = '新建投放计划'
      break
    default:
      break
  }
